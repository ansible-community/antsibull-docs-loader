...
def typing(session: nox.Session):
    ...
    session.run("mypy", "src/antsibull_docs_loader", "tests")
...
def bump(session: nox.Session):
    ...
    if len(session.posargs) > 1:
        ...
        session.run(
            "git",
            "add",
            "src/antsibull_docs_loader/__init__.py",
            str(fragment_file),
            external=True,
        )
    ...
    session.run(
        ...
        # src/antsibull_docs_loader/__init__.py is not committed in the last step
    ...
    session.run(
        ...
        "-a",
        "-m",
        f"antsibull-docs-loader {version}",
        "--edit",
        version,
        external=True,
    )
...
def publish(session: nox.Session):
    ...
    session.run("git", "add", "src/antsibull_docs_loader/__init__.py", external=True)
